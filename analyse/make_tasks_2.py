...
def main(sentences, posts, n_qa, n_rte, pdf, seed):
    ...
    outfile_QA = 'temp12345_qa_small.tsv'   # TODO: Make these cmd line args?
    outfile_RTE = 'temp12345_rte_small.tsv'
    ...
    write_pairs_to_squad_format(pairs_QA, all_user_posts, outfile_QA)
...
def write_table(df, pdf=None):
    ...
    if pdf:
        logging.info(f'Writing table to PDF {pdf}.')
...
def write_pairs_to_squad_format(pairs, posts, outfile):
    """
    Write pairs of questions and pivot positions to a file in SQUADv2 format.

    Args:
        pairs (list): List of tuples containing question and pivot information.
    ...
    with open(outfile, 'w', newline='') as file:
        ...
        for (question, pivot) in pairs:
            ...
            new_start= new_post.find(pivot_text)
            new_end = new_start + len(pivot_text)
